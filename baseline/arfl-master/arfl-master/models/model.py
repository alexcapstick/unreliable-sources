...
class FedModel(ABC):
    ...
    def train(self, data, num_epochs, batch_size, lr):
        ...
        if self.datagen:
            ...
            self.net.fit(
                features, labels, batch_size=batch_size, verbose=0, epochs=num_epochs
            )
    ...
    def test(self, data):
        ...
        loss, acc, top5 = self.net.evaluate(
            features, labels, verbose=0, batch_size=self.test_bs
        )
        print(
            "prediction shape:",
            self.net.predict(features, batch_size=self.test_bs).shape,
            "labels shape: ",
            labels.shape,
        )

        return {ACCURACY_KEY: acc, "top_5_acc": top5, "loss": loss}
