...
DATASETS = ["sent140", "femnist", "shakespeare", "cifar10", "fmnist"]
...
def parse_args():
    ...
    parser.add_argument(
        "--method", help="name of dataset;", choices=METHODS, type=str, default="arfl"
    )

    parser.add_argument(
        "--corruption-name", help="Type of corruption used.", type=str, default=""
    )

    parser.add_argument(
        "--model", help="name of model;", type=str, default="cnn", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reg_weight", help="weight of regularization term", type=float, required=False
    )
    parser.add_argument(
        "-pc", help="proportion of clients to be corrupted", type=float, default=0.0
    )
    parser.add_argument(
        "-ps",
        help="proportion of sample in each client to be corrupted",
        type=float,
        default=1.0,
    )
    parser.add_argument(
        "--setup_clients", help="proportion of setup clients", type=float, default=1.0
    )
    parser.add_argument("--attack_type", help="Attacking option", type=int, default=3)
    parser.add_argument(
        "--num_actors", help="total number of actors", type=int, default=4
    )
    ...
    load_flag_parser.add_argument(
        "--no-loadmodel", dest="loadmodel", action="store_false"
    )
