...
writer = SummaryWriter(
    comment="-ARFL-{}-seed_{}-{}".format(ARGS.corruption_name, ARGS.seed, ARGS.dataset)
)
...
def main():
    ...
    log_filename = os.path.join(
        args.metrics_dir, args.metrics_name + "_" + args.model + ".csv"
    )
    ...
    client_ids, client_groups, num_train_samples, num_test_samples = (
        manager.get_clients_info()
    )
    ...
    for i in range(num_rounds):
        ...
        server.train_model(
            num_epochs=args.num_epochs, batch_size=args.batch_size, round=i
        )
...
def print_metrics(metrics, weights, step, prefix=""):
    ...
    for metric in metric_names:
        ...
        key_metric = {c: metrics[c][metric] for c in sorted(metrics)}
        writer.add_scalar(
            "weighted_average_{}results_{}".format(prefix, metric),
            np.average(ordered_metric, weights=ordered_weights),
            step,
        )
        writer.add_scalars("{}results_{}".format(prefix, metric), key_metric, step)
        writer.add_scalars("{}weights".format(prefix), weights, step)
